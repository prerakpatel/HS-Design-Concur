...
| **Version files** | Each format keeps the files of its two newest versions (and of any approved version). When a new version arrives, older ones lose their files but keep their row, decision and comments, so the thread stays readable; the version pill turns dashed and the page says why. The daily job re-checks. |
