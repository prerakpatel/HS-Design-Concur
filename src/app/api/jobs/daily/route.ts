 ...
 * 2. Daily digest email for users whose email_pref is "digest".
 * Retention (archive / purge / draft sweep) is added here in step 6.
 */
export async function GET(req: Request) {
  ...
  const report: Record<string, number> = { reminders: 0, digests: 0 };
  ...
  if (emailConfigured()) {
    ...
    for (const u of users ?? []) {
      ...
      if ("ok" in r) { await db.from("notifications").update({ emailed_at: new Date().toISOString() }).in("id", items.map((n) => n.id)); report.digests++; }
    }
    // Instant users also get due-date reminders by email (created above without the request-time delivery path).
    const { data: instant } = await db.from("notifications").select("id,user_id,kind,payload,users!inner(email,email_pref,status)").eq("kind", "slot.due").is("emailed_at", null).eq("users.email_pref", "instant").eq("users.status", "active");
    for (const n of instant ?? []) {
      const u = n.users as unknown as { email: string }; const p = n.payload as Record<string, unknown>;
      const r = await sendEmail(u.email, `${p.when === "today" ? "Due today" : "Due in 3 days"}: ${p.format} for ${p.title}`, { heading: notificationText("slot.due", p), body: String(p.title ?? ""), cta: { label: "Open the asset", href: appUrl(notificationHref(p)) } });
