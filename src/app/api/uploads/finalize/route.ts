...
export async function POST(req: Request) {
  ...
  const body = await req.json().catch(() => null) as { slotId?: string; tmpPath?: string; mime?: string; side?: "front" | "back" } | null;
  ...
  const result = await finalizeUploadFor(ctx, body.slotId, body.tmpPath, body.mime, body.side === "back" ? "back" : "front");
