...
export default async function PreviewPage({ params }: { params: Promise<{ screen: string }> }) {
  ...
  if (screen === "wizard-formats") return <WizardShell eventId="e-4" step="formats" statuses={{ basics: "done", brief: "incomplete" }} title="Which formats does this event need?" subtitle="All formats start off. Turn on the ones you need." wide><FormatsForm eventId="e-4" rows={F.FORMAT_ROWS} action={noopForm} /></WizardShell>;
