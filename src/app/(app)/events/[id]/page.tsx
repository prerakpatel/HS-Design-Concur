...
export default async function EventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: brief }, { data: timings }, { data: slots }, { data: formats }, { data: activity }, { data: creator }] = await Promise.all([
    ...
    supabase.from("slots").select("*,assignee:assignee_id(name,email),versions(number,version_sides(side,thumb_path,reference_path))").eq("event_id", id),
  ...
  const cards: FormatCardData[] = (await Promise.all((formats ?? []).map(async (f) => {
    const slot = bySlotFormat.get(f.id) as (Slot & { assignee: { name: string | null; email: string } | null; versions: { number: number; version_sides: { side: string; thumb_path: string | null; reference_path: string | null }[] }[] }) | undefined;
    if (!slot) return null;
