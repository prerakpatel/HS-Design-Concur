...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <AppShell org={org} orgs={orgs} user={{ name: user.name ?? user.email, role, initials: initials(user.name, user.email), avatar: user.avatar_url }} slots={{ used: count ?? 0, max: EVENT_CAP }} sidebarOpen={sidebarOpen}>{children}</AppShell>;
}
