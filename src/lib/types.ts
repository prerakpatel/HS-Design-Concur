...
export interface Organization { id: string; slug: string; name: string; short_name: string; accepting_signups: boolean; email_enabled: boolean; chat_enabled: boolean; chat_webhook_url: string | null; slack_enabled: boolean; slack_webhook_url: string | null; logo_path: string | null; }
export interface AppUser { id: string; email: string; name: string | null; avatar_url: string | null; role: UserRole; is_approver: boolean; function_tags: FunctionTag[]; status: UserStatus; email_pref: "instant" | "digest" | "off"; slack_user_id: string | null; gchat_user_id: string | null; changelog_seen: string | null; created_at: string; }
