      ...
      <aside className="hidden w-[220px] shrink-0 border-r border-border bg-subtle p-5 md:block">
        ...
        <ol className="mt-3 space-y-1">
          {STEPS.map((s, i) => {
            ...
            return (
              <li key={s.key}>
                <Link href={`/events/${eventId}/edit/${s.key}`} className={"flex h-10 items-center gap-3 rounded-lg px-3 text-sm font-medium " + (state === "active" ? "bg-muted text-foreground" : state === "done" ? "text-foreground" : "text-muted-foreground")}>
