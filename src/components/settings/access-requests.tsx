...
export function AccessRequests({ pending, orgs, defaultOrgId, action }: { pending: PendingUser[]; orgs: { id: string; label: string }[]; defaultOrgId?: string; action: (userId: string, formData: FormData) => Promise<void> }) {
  if (pending.length === 0) return <p className="rounded-2xl border border-dashed border-border px-6 py-12 text-center text-sm text-muted-foreground">No one is waiting. New sign-ins appear here.</p>;
  return (
    <ul className="space-y-4">
      {pending.map((u) => (
        <li key={u.id}>
          <form action={action.bind(null, u.id)} className="rounded-2xl border border-border p-5 md:p-6">
            <div className="flex items-center gap-4"><UserAvatar initials={u.initials} size={40} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{u.name}</p><p className="text-sm text-muted-foreground"><span className="break-all">{u.email}</span> · asked {relativeTime(u.requested_at)}</p></div></div>
